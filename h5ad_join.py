...
def interface():
    ...
    parser.add_argument('H5AD_FILES',
                        type=str,
                        nargs='+',
                        metavar='<H5AD_FILES>',
                        help='Loom file.')
...
if __name__ == '__main__':
    ...
    if args.ids:
        assert len(args.ids) == len(args.H5AD_FILES), 'Length of IDs must be the same as the number of H5AD files'
    else:
        args.ids = [f'SAMPLE_{i}' for i in range(len(args.H5AD_FILES))]

    ens2sym = pd.read_csv(ENS2SYM, sep='\t')

    adata_list = list()
    for name, h5ad in zip(args.ids, args.H5AD_FILES):
