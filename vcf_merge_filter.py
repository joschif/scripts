...
def interface():
    ...
    parser.add_argument("vcffile", help="The VCF file (- for STDIN)")
    parser.add_argument(
        "--homo",
        dest="only_homo",
        action="store_true",
        default=DEFAULTS["only_homo"],
        help="Only consider the variant sites with all lines homozygous",
    )
    parser.add_argument(
        "--allow-unknown",
        dest="allow_unknown",
...
if __name__ == "__main__":
    ...
    for line in fh:
        ...
        unknown_genotypes = np.array([])
        line_genotypes = list()
        for a in np.unique(annot):
            annot_idx = np.where(annot == a)[0]
            annot_gt = genotypes[annot_idx]
            unknown_genotypes = np.append(unknown_genotypes, np.any(annot_gt == "./."))
        ...
        if not consistent_genotypes:
            continue

        # Skip if more lines have unknown sites than allow_unknown
        if unknown_genotypes.sum() > args.allow_unknown:
            continue
